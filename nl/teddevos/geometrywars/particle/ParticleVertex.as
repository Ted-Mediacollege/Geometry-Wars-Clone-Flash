package nl.teddevos.geometrywars.particle
{
    ...
    {
        public static const ELEMENTS_PER_VERTEX:int = 13;
        public static const POSITION_OFFSET:int = 0; 		//x, y
        public static const COLOR_OFFSET:int = 2;			//r, g, b, a
        public static const VELOCITY_OFFSET:int = 6;		//vx, vy, age
        public static const SCALE_OFFSET1:int = 9;			//w, h,
        public static const SCALE_OFFSET2:int = 11;			//h, w,
        ...
        public function ParticleVertex(length:int, premultipliedAlpha:Boolean=false)
        {
            mRawData = new <Number>[];
        ...
        public function copyTo(targetData:ParticleVertex, targetVertexID:int=0, vertexID:int=0, numVertices:int=-1):void
        {
            copyTransformedTo(targetData, targetVertexID, null, vertexID, numVertices);
        }
        
        public function copyTransformedTo(targetData:ParticleVertex, targetVertexID:int=0, matrix:Matrix=null, vertexID:int=0, numVertices:int=-1, vx:Number = 1, vy:Number = 0, age:Number = 100):void
        {
            ...
            if (matrix)
            {
                ...
                {	
                    ...
                    targetRawData[int(targetIndex++)] = vx; sourceIndex++; //vx
                    targetRawData[int(targetIndex++)] = vy; sourceIndex++; //vy
                    targetRawData[int(targetIndex++)] = age; sourceIndex++; //age
                    targetRawData[int(targetIndex++)] = count % 2 == 1 ? 1 : 0; sourceIndex++; //w
                    targetRawData[int(targetIndex++)] = count % 4 > 1 ? 1 : 0; sourceIndex++; //h
                    targetRawData[int(targetIndex++)] = count % 4 > 1 ? 1 : 0; sourceIndex++; //w
                    targetRawData[int(targetIndex++)] = count % 2 == 1 ? 1 : 0; sourceIndex++; //h
					count++;
                }
            }
            else
        ...
        public function append(data:ParticleVertex):void
        {
        ...
        public function setVelocity(vertexID:int, x:Number, y:Number):void
        {
            var offset:int = vertexID * ELEMENTS_PER_VERTEX + VELOCITY_OFFSET;
            mRawData[offset]        = x;
            mRawData[int(offset+1)] = y;
        }
        
        /** Returns the texture coordinates of a vertex in the range 0-1. */
        public function getVelocity(vertexID:int, velocity:Point):void
        {
            var offset:int = vertexID * ELEMENTS_PER_VERTEX + VELOCITY_OFFSET;
            velocity.x = mRawData[offset];
            velocity.y = mRawData[int(offset+1)];
        }
		
        /** Returns the texture coordinates of a vertex in the range 0-1. */
        public function getSize(vertexID:int, s:Point):void
        {
            var offset:int = vertexID * ELEMENTS_PER_VERTEX + SCALE_OFFSET1;
            s.x = mRawData[offset];
            s.y = mRawData[int(offset+1)];
        }
        ...
        {
            var result:String = "[ParticleVertex \n";
