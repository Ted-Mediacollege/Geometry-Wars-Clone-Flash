package nl.teddevos.geometrywars.event.waves 
{
	...
	public class EventWavesDoubleFull extends EventBase
	{
		public var random:int = 0;
		
		public function EventWavesDoubleFull(t:int, r:int) 
		{
		...
		override public function start(world:World):void
		{
			...
			if (world.player.y > -200)
			{
				...
				{
					spawnRow(world, false, 2);
				}
				else
				{
					spawnRow(world, false, 0);
				}
			}
			else
			{
			...
			if (world.player.x < 450)
			{
				...
				{
					spawnRow(world, false, 3);
				}
				else
				{
					spawnRow(world, false, 1);
				}
			}
			else
			{
